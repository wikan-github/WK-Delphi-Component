...
type
  ...
  TWKPngImageList = class(TImageList)
  private
...
procedure Register;
begin
  RegisterComponents('WKComponent', [TWKPngImageList]);
end;
...
begin
  if ImageListCount = 0 then begin
    ...
    Pointers[0] := Combo(@TCustomImageList.Add, @TWKPngImageList.Add, 'Add');
    Pointers[1] := Combo(@TCustomImageList.AddIcon, @TWKPngImageList.AddIcon, 'AddIcon');
    Pointers[2] := Combo(@TCustomImageList.AddImage, @TWKPngImageList.AddImage, 'AddImage');
    Pointers[3] := Combo(@TCustomImageList.AddImages, @TWKPngImageList.AddImages, 'AddImages');
    Pointers[4] := Combo(@TCustomImageList.AddMasked, @TWKPngImageList.AddMasked, 'AddMasked');
    Pointers[5] := Combo(@TCustomImageList.Clear, @TWKPngImageList.Clear, 'Clear');
    Pointers[6] := Combo(@TCustomImageList.Delete, @TWKPngImageList.Delete, 'Delete');
    Pointers[7] := Combo(@TCustomImageList.Insert, @TWKPngImageList.Insert, 'Insert');
    Pointers[8] := Combo(@TCustomImageList.InsertIcon, @TWKPngImageList.InsertIcon, 'InsertIcon');
    Pointers[9] := Combo(@TCustomImageList.InsertMasked, @TWKPngImageList.InsertMasked, 'InsertMasked');
    Pointers[10] := Combo(@TCustomImageList.Move, @TWKPngImageList.Move, 'Move');
    Pointers[11] := Combo(@TCustomImageList.Replace, @TWKPngImageList.Replace, 'Replace');
    Pointers[12] := Combo(@TCustomImageList.ReplaceIcon, @TWKPngImageList.ReplaceIcon, 'ReplaceIcon');
    Pointers[13] := Combo(@TCustomImageList.ReplaceMasked, @TWKPngImageList.ReplaceMasked, 'ReplaceMasked');
...
constructor TWKPngImageList.Create(AOwner: TComponent);
begin
...
destructor TWKPngImageList.Destroy;
begin
  FPngImages.Free;
  Dec(ImageListCount);
  if ImageListCount = 0 then
...
function TWKPngImageList.Add(Image, Mask: TBitmap): Integer;
var
  Patch: TMethodPatch;
  Png: TPngImage;
begin
  if TObject(Self) is TWKPngImageList then begin
...
function TWKPngImageList.AddIcon(Image: TIcon): Integer;
var
  Patch: TMethodPatch;
  Png: TPngImage;
begin
...
function TWKPngImageList.AddPng(Image: TPngImage; Background: TColor = clNone):
    Integer;
var
...
function TWKPngImageList.AddImage(Value: TCustomImageList; Index: Integer): Integer;
var
  Patch: TMethodPatch;
  Png: TPngImage;
begin
...
procedure TWKPngImageList.AddImages(Value: TCustomImageList);
var
  Patch: TMethodPatch;
  I: Integer;
  Png: TPngImage;
...
function TWKPngImageList.AddMasked(Image: TBitmap; MaskColor: TColor): Integer;
var
  Patch: TMethodPatch;
  Png: TPngImage;
begin
...
procedure TWKPngImageList.AssignTo(Dest: TPersistent);
begin
  inherited;
  if Dest is TWKPngImageList then begin
    TWKPngImageList(Dest).PngImages := FPngImages;
    TWKPngImageList(Dest).EnabledImages := FEnabledImages;
  end;
end;

procedure TWKPngImageList.BeginUpdate;
begin
  Inc(FLocked);
end;

procedure TWKPngImageList.Clear;
var
  Patch: TMethodPatch;
begin
  if TObject(Self) is TWKPngImageList then begin
...
procedure TWKPngImageList.CopyPngs;
var
  I: Integer;
  Png: TPngImage;
  Icon: HIcon;
...
procedure TWKPngImageList.Delete(Index: Integer);
var
  Patch: TMethodPatch;
begin
  if TObject(Self) is TWKPngImageList then begin
...
procedure TWKPngImageList.DoDraw(Index: Integer; Canvas: TCanvas; X, Y: Integer; Style: Cardinal; Enabled: Boolean);
var
  PaintRect: TRect;
  Options: TPngOptions;
...
procedure TWKPngImageList.EndUpdate(Update: Boolean);
begin
  Dec(FLocked);
...
function TWKPngImageList.GetHeight: Integer;
begin
  Result := inherited Height;
end;

function TWKPngImageList.GetWidth: Integer;
begin
  Result := inherited Width;
end;

procedure TWKPngImageList.Insert(Index: Integer; Image, Mask: TBitmap);
var
  Patch: TMethodPatch;
  Png: TPngImage;
begin
...
procedure TWKPngImageList.InsertIcon(Index: Integer; Image: TIcon);
var
  Patch: TMethodPatch;
  Png: TPngImage;
begin
...
procedure TWKPngImageList.InsertPng(Index: Integer; Image: TPngImage; Background:
    TColor = clNone);
var
  Item: TPngImageCollectionItem;
begin
...
procedure TWKPngImageList.InsertMasked(Index: Integer; Image: TBitmap; MaskColor: TColor);
var
  Patch: TMethodPatch;
  Png: TPngImage;
begin
...
procedure TWKPngImageList.InternalInsertPng(Index: Integer; const Png: TPngImage;
    Background: TColor);
var
  I: Integer;
  Icon: HICON;
  TempList: TWKPngImageList;
begin
  TempList := TWKPngImageList(TComponentClass(ClassType).Create(nil));
  try
...
procedure TWKPngImageList.InternalAddPng(const Png: TPngImage; Background: TColor
    = clNone);
var
  Icon: HICON;
begin
...
procedure TWKPngImageList.Move(CurIndex, NewIndex: Integer);
var
  Patch: TMethodPatch;
begin
  if TObject(Self) is TWKPngImageList then begin
...
function TWKPngImageList.PngToIcon(const Png: TPngImage; Background: TColor): HICON;
const
...
procedure TWKPngImageList.ReadData(Stream: TStream);
begin
  //Make sure nothing gets read from the DFM
end;

procedure TWKPngImageList.Replace(Index: Integer; Image, Mask: TBitmap);
var
  Item: TPngImageCollectionItem;
  Patch: TMethodPatch;
  Icon: HICON;
...
procedure TWKPngImageList.ReplaceIcon(Index: Integer; Image: TIcon);
var
  Item: TPngImageCollectionItem;
  Patch: TMethodPatch;
  Icon: HICON;
...
procedure TWKPngImageList.ReplaceMasked(Index: Integer; NewImage: TBitmap; MaskColor: TColor);
var
  Item: TPngImageCollectionItem;
  Patch: TMethodPatch;
  Icon: HICON;
...
procedure TWKPngImageList.SetEnabledImages(const Value: Boolean);
begin
...
procedure TWKPngImageList.SetHeight(const Value: Integer);
begin
...
procedure TWKPngImageList.SetPngImages(const Value: TPngImageCollectionItems);
begin
...
procedure TWKPngImageList.SetPngOptions(const Value: TPngOptions);
begin
...
procedure TWKPngImageList.SetWidth(const Value: Integer);
begin
...
procedure TWKPngImageList.WriteData(Stream: TStream);
begin
...
begin
  ...
  if FOwner is TWKPngImageList then
    TWKPngImageList(FOwner).CopyPngs;
end;
